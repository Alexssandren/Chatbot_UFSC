...
export function AcademicSummaryCard({ summary }: Props) {
  const eligibleClass = summary.eligible
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
      ...
      <div className={`mb-6 rounded-lg border px-4 py-3 text-sm font-medium ${eligibleClass}`}>
        {summary.eligible ? 'Elegível' : 'Não elegível'}
      </div>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-2">
        ...
        <div>
          <p className="text-sm text-gray-500">Horas restantes (até mínimo total)</p>
          <p className="text-lg font-semibold text-gray-900">{summary.remainingEligibleHours}h</p>
        </div>
        <div>
      ...
      <h4 className="mb-2 text-sm font-semibold uppercase tracking-wide text-gray-600">Grupos (GI–GV)</h4>
      <ul className="mb-6 space-y-2">
        {summary.groups.map((g) => {
          const shortfall = Math.max(0, g.minimumRequiredHours - g.eligibleHours);
          ...
          return (
            ...
            >
              ...
              {g.meetsMinimumHours ? (
                ...
                <span className="mt-1 block text-amber-800">Faltam {shortfall}h</span>
              )}
            </li>
          );
        })}
